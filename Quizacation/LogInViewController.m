...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *large_g_charac_height;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *password_btn_top;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *submit_btn_top;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *not_registered_btn_top;
...
@property (weak, nonatomic) IBOutlet UIButton *not_resistered_btn;
...
@synthesize title_label_top, large_q_charac_top, large_g_charac_height, password_btn_top, submit_btn_top, not_registered_btn_top, student_id_field, password_field, submit_btn, not_resistered_btn, q_charac_bottom;
...
- (void) adjust_constraints
{
    if (g_GameSet.g_curDeviceModel == IPHONE_5) {
        ...
        self.large_g_charac_height.constant *= 1.2f;
        self.password_btn_top.constant *= 1.2f;
        self.submit_btn_top.constant *= 1.2f;
        self.not_registered_btn_top.constant *= 1.2f;
        ...
        [self.password_field setFont:[UIFont systemFontOfSize:30]];
         
        [self.submit_btn.titleLabel setFont:[UIFont systemFontOfSize:30]];
        [self.not_resistered_btn.titleLabel setFont:[UIFont systemFontOfSize:25]];
    }
    else if (g_GameSet.g_curDeviceModel == IPHONE_6_PLUS) {
        self.large_q_charac_top.constant *= 1.3f;
        self.large_g_charac_height.constant *= 1.3f;
        self.password_btn_top.constant *= 1.3f;
        self.submit_btn_top.constant *= 1.3f;
        self.not_registered_btn_top.constant *= 1.3f;
        ...
        self.large_g_charac_height.constant *= 0.95f;
        self.password_btn_top.constant *= 0.8f;
        self.submit_btn_top.constant *= 0.8f;
        self.not_registered_btn_top.constant *= 0.8f;
...
//        [self.password_field becomeFirstResponder];
    ...
    else if( self.password_field == textField )
    {
...
- (IBAction)onSubmitBtn:(id)sender {
    if (self.m_string_student_id == nil || self.m_string_password == nil) {
        if ([UIAlertController class])
        {
            
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"" message:@"Please enter log in infomation." preferredStyle:UIAlertControllerStyleAlert];
...
//            [self presentViewController:alertController animated:YES completion:nil];
        ...
        else
        {
            ...
            UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"" message:@"Please enter log in infomation." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        ...
        [self loginWithUsername:self.m_string_student_id Password:self.m_string_password];
    }
}

- (void)loginWithUsername:(NSString *)uname Password:(NSString *)pword
{
    
    NSString *userName = uname;//@"abc";//@"admin@admin.com";
    NSString *password = pword;//@"abc";//@"password";
    NSString *token = g_GameSet.deviceToken;
    
    NSString *strReqeust = [NSString stringWithFormat:@"studentid=%@&password=%@&tokenid=%@",
                            userName,
                            password,
                            token
                            ];
...
//                                     }
//
//                                     g_GameSet.g_cur_username = self.student_id_field.text;
//                                     
//                                     LogInSuccessViewController *login_success =
...
//            [alertController addAction:ok];
//
//            g_GameSet.g_user_id = [data objectAtIndex:0];
//            g_GameSet.g_blog_id = [data objectAtIndex:1];
//            
//            [self presentViewController:alertController animated:YES completion:nil];
