...
@interface SpecialRespViewController ()
...
@implementation SpecialRespViewController
...
- (void)viewDidLoad {
    ...
    question_label.text = g_GameSet.g_specialQuiz.title;
    v_quiz_array = g_GameSet.g_specialQuiz.answers_arr;
    self.m_cur_category = g_GameSet.g_specialQuiz.category;
    
    self.m_correct_index = [self getIndexOfCorrectAnswer:g_GameSet.g_specialQuiz.solution];
...
- (void) processTimeEnd
{
    ...
    quiz_result.quiz_title = g_GameSet.g_specialQuiz.title;
    ...
    [g_GameSet.g_special_quiz_result_Arr addObject:quiz_result];
    ...
    if (g_GameSet.g_current_ques_index == [g_GameSet.g_specialQuiz_Arr count] - 1) {
        
        g_GameSet.g_current_ques_index = 0;
        [g_GameSet.g_specialQuiz_Arr removeAllObjects];
        
        SpecialResultOverViewController *resultView =
        (SpecialResultOverViewController *)
        [storyboard instantiateViewControllerWithIdentifier:@"SpecialResultOverViewController"];
        [self showViewController:resultView sender:nil];
        
    }else{
        
        g_GameSet.g_current_ques_index++;
        //////sending category : mark
        
        NSDictionary *v_quiz = [g_GameSet.g_specialQuiz_Arr objectAtIndex:g_GameSet.g_current_ques_index];
        
        NSString *problem_title = [v_quiz objectForKey:@"title"];
        NSMutableArray  *answers_arr = [v_quiz objectForKey:@"answers"];
...
//        SpecialRespViewController *resultView =
...
-(int) getIndexOfCorrectAnswer : (NSString*) answerStr
{
    ...
    solution_str = g_GameSet.g_specialQuiz.solution;
...
{
    //////mmm mmm mmm
    NSString *strReqeust = [NSString stringWithFormat:@"user_id=%@&blog_id=%@&request_type=3&category=%@&mark=%d",g_GameSet.g_user_id, g_GameSet.g_blog_id, category_id, mark];
