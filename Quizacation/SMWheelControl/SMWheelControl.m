...
///////mmm mmm mmm
static const CGFloat kMaxVelocity = 90.0;//20.0;
static const CGFloat kMinVelocity = 40.0;//20.0;
static const CGFloat kSMDecelerationMultiplier = 0.99;//0.97;
...
-(void) spinRotatingStart
{
    
    _startTouchTime = _endTouchTime;
    _endTouchTime = CACurrentMediaTime();

    CGFloat angleDelta = 10.0f;
    self.sliceContainer.transform = CGAffineTransformRotate(self.sliceContainer.transform, angleDelta);
...
- (void)beginDeceleration
{
    ...
    NSLog(@"-------_animatingVelocity   %f-----", _animatingVelocity);
...
//    }
    ...
    ///mmm mmm mmm testtesttest
    velocity = arc4random() % 11 * 0.1 * 90.0f;//90.00f;
